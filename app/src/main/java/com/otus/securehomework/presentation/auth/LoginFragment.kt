...
class LoginFragment : Fragment(R.layout.fragment_login) {
            ...
            biometric.isUserRegistered().asLiveData().observe(viewLifecycleOwner) {
                ...
                if (isUserRegistered) {
                    ...
                    binding.useBiometrics.setOnClickListener {
                        findNavController().navigate(R.id.action_loginFragment_to_biometricFragment)
                    }
                }
            }
        }
    ...
    private fun onUseBiometric() {
        ...
        val onAuthenticateUser = {
            biometric.authenticateUser(requireActivity(), getString(R.string.biometric_prompt_use_password_instead)) {
                userConnectionData: CharSequence ->
        ...
        binding.useBiometrics.setOnClickListener { onAuthenticateUser() }
        onAuthenticateUser()
        ...
        binding.clearBiometrics.setOnClickListener {
            biometric.authenticateUser(requireActivity(), getString(R.string.cancel)) {
                _: CharSequence -> lifecycleScope.launch { biometric.clearRegisteredUser() }
            }
        }
    }

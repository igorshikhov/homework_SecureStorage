...
class AesKeysPreferences (
    private val applicationContext: Context,
    private val rsaParameterSpec: IRSAParameterSpec,
) : AesKey() {
    companion object {
    ...
    private fun getRsaPrivateKey(): PrivateKey {
        return keyStore.getKey(IRSAParameterSpec.RSA_KEY_ALIAS, null) as? PrivateKey ?: generateRsaSecretKey().private
    }

    private fun getRsaPublicKey(): PublicKey {
        return keyStore.getCertificate(IRSAParameterSpec.RSA_KEY_ALIAS)?.publicKey?: generateRsaSecretKey().public
    }

    private fun generateRsaSecretKey(): KeyPair {
        return KeyPairGenerator.getInstance(RSA_ALGORITHM, KEY_PROVIDER).run {
            initialize(rsaParameterSpec.getAlgorithmParameterSpec())
            generateKeyPair()
        }
    }
}